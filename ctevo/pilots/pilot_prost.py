...
def get_feature_embedding(species):
    fn_embedding = '../data/prost/embeddings_prost.h5'
...
def get_avg_expression(species):
    ...
    res = pd.DataFrame(res['avg'].T, columns=mi, index=features)
...
def get_celltype_embedding(organism):
    ...
    # Filter out NaNs and get intersection
    features_shared = list(set(avg.coords['dim_0'].values) & set(gemb.coords['dim_0'].values))
    avg = avg.loc[features_shared]
    gemb = gemb.loc[features_shared]
    ...
    for (ct, organ, _) in avg.coords['dim_1'].values:
        ...
        idx_sort = diff.sortby(diff, ascending=False).dim_0.values
        # Decreasing importance, linear to zero
        for i in range(nmarkers):
            import ipdb; ipdb.set_trace()
            avg2.loc[idx_sort[i], ct] *= 1.0 * (nmarkers - i) / nmarkers
        avg2.loc[idx_sort[nmarkers:], ct] = 0
    avg = avg2
